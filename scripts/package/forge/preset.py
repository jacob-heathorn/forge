...
class Preset:
  ...
  def build(self, debug: bool, verbose: bool):
    with pushd(self._preset_build_root(debug)):
      # Configure
      args = ['cmake', '-G', self.generator, self.project_root, f'-DCMAKE_TOOLCHAIN_FILE={self.cmake_toolchain_file}', f'-DCMAKE_BUILD_TYPE={cmake_build_type(debug)}', f'-DCMAKE_EXPORT_COMPILE_COMMANDS={self.cmake_export_compile_commands}'
              ]
  ...
  def run(self, executable: str):
    fullfile = find_application(executable, self._preset_build_root(debug=False))
    self.debugger.run(fullfile)
    print_size(fullfile)

  def debug(self, executable: str):
    self._check_debugger()
    fullfile = find_application(executable, self._preset_build_root(debug=True))
    print_size(fullfile)
